...
def parse_args(query,mode,mode_act):
    ...
    parser.add_argument("--model", type=str, default="/share/xieruiqi-local/sdxl-turbo/")
    ...
    args.prompt = query
    args.model = "/share/xieruiqi-local/sdxl-turbo/"
    args.ckpt = "/home/xieruiqi/ComfyUI/custom_nodes/MixDQ/kernels/output/new_ckpt.pth"
    args.output_type = "pil"
    if mode == "W8-bit(Recommended)":
        args.w_config = "/home/xieruiqi/ComfyUI/custom_nodes/MixDQ/kernels/cfgs/weight/weight_8.00.yaml"
    else:
        args.w_config = "/home/xieruiqi/ComfyUI/custom_nodes/MixDQ/kernels/cfgs/weight/weight_5.02.yaml"
    if mode_act == "W8-bit":
        args.a_config = "/home/xieruiqi/ComfyUI/custom_nodes/MixDQ/kernels/cfgs/act/act_8.00.yaml"
    if mode_act == "W7.84-bit":
        args.a_config = "/home/xieruiqi/ComfyUI/custom_nodes/MixDQ/kernels/cfgs/act/act_7.84.yaml"
    else:
        args.a_config = None
    args.quantize = True
    args.batch_size = 1
...
def run(pipeline, args):
    ...
    if args.run_pipeline:
        if args.quantize:
            latents.save("/home/xieruiqi/ComfyUI/custom_nodes/MixDQ/pictures/gen.png")
        else:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "weight_mode": (["W8-bit(Recommended)","W5.02-bit"],),
                "act_mode": (["None(Recommended)","W8-bit","W7.84-bit"],)
            }
        }
    RETURN_TYPES = ("IMAGE",)
    FUNCTION = "mixdq_quant"

    def mixdq_quant(self,query,weight_mode,act_mode):
        seed_everything(42)
        print("DEBUG: Your choosing mode for quantilization(W A):")
        print(weight_mode)
        print(act_mode)

        args = parse_args(query,weight_mode,act_mode)
        ...
        if args.quantize:
            ...
            bos_dict = torch.load("/home/xieruiqi/ComfyUI/custom_nodes/MixDQ/kernels/bos_pre_computed.pt", map_location = 'cpu')  
        ...
        image_to_next=run(pipeline, args)
        save_path = "/home/xieruiqi/ComfyUI/custom_nodes/MixDQ/pictures/gen.png"
